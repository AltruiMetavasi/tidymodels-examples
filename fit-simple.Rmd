...
```{r data-initial_split}
# create initial split
splitted <- initial_split(data_clean, prop = 0.8, strata = "response")
...
rec <- recipe(response ~ ., data = training(splitted)) %>% 
  step_rm(duration) %>%
  step_nzv(all_predictors()) %>% 
  step_string2factor(all_nominal(), -response) %>%
  step_string2factor(response, levels = c("yes", "no")) %>%
  step_downsample(response, ratio = 1/1, seed = 100) %>%
...
```{r model-fit}
# fit the model
model <- fit_xy(model_spec, x = juice(rec, -response), y = juice(rec, response))
...
pred_test %>%
  summarise(
    accuracy = accuracy_vec(response, .pred_class),
    sensitivity = sens_vec(response, .pred_class),
    specificity = spec_vec(response, .pred_class),
    precision = precision_vec(response, .pred_class)
  )
```
